...
void demo_update_data_buffer(struct demo *demo) {
    ...
    mat4x4_rotate_X(demo->model_matrix, Model, (float)degreesToRadians(demo->spin_angle));
    mat4x4_dup(Model, demo->model_matrix);
    ...
    *height = 256;
    ...
    for (int y = 0; y < *height; y++) {
        ...
        for (int x = 0; x < *width; x++) {
            *((uint32_t *)rowPtr) = arc4random();
            rowPtr[0] = (rowPtr[0] + 127) / 2;
            rowPtr[1] = (rowPtr[1] + 127) / 2;
            rowPtr[2] = (rowPtr[2] + 127) / 2;
            rowPtr += 4;
        }
